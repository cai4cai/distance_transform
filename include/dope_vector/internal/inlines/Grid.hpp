...
#ifndef INCLUDE_DOPE_VECTOR_INTERNAL_INLINES_GRID_HPP_
#define INCLUDE_DOPE_VECTOR_INTERNAL_INLINES_GRID_HPP_
...
inline Grid<T, Dimension, Allocator>::Grid(const IndexD &size,
...
template <typename T, SizeType Dimension, class Allocator>
inline Grid<T, Dimension, Allocator>::Grid(const SizeType size,
                                           const T &default_value)
    : _data(Index<Dimension>::Constant(size).prod(), default_value) {
  DopeVector<T, Dimension>::reset(_data.data(), static_cast<SizeType>(0),
                                  Index<Dimension>::Constant(size));
}
...
inline Grid<T, Dimension, Allocator>::Grid(const SizeType size,
  ...
  DopeVector<T, Dimension>::reset(_data.data(), static_cast<SizeType>(0),
                                  Index<Dimension>::Constant(size), offset);
}

template <typename T, SizeType Dimension, class Allocator>
inline Grid<T, Dimension, Allocator>::Grid(const Grid &o) : _data(o._data) {
  DopeVector<T, Dimension>::reset(_data.data(), static_cast<SizeType>(0),
                                  o.allSizes());
}
...
template <typename T, SizeType Dimension, class Allocator>
inline const typename Grid<T, Dimension, Allocator>::Data &
Grid<T, Dimension, Allocator>::to_stdvector() const {
  return _data;
}

template <typename T, SizeType Dimension, class Allocator>
inline typename Grid<T, Dimension, Allocator>::Data &
Grid<T, Dimension, Allocator>::to_stdvector() {
  return _data;
}
...
inline bool Grid<T, Dimension, Allocator>::operator==(const Grid &o) const {
  ...
  for (SizeType i = static_cast<SizeType>(0); i < Dimension; ++i)
    if (DopeVector<T, Dimension>::sizeAt(i) != o.sizeAt(i)) return false;
...
inline void Grid<T, Dimension, Allocator>::clear() {
  _data.clear();
  DopeVector<T, Dimension>::reset(nullptr, static_cast<SizeType>(0),
                                  IndexD::Zero());
}
...
template <typename T, SizeType Dimension, class Allocator>
inline void Grid<T, Dimension, Allocator>::resize(const IndexD &size,
                                                  const T &default_value) {
...
template <typename T, SizeType Dimension, class Allocator>
inline void Grid<T, Dimension, Allocator>::resize(const SizeType size,
                                                  const T &default_value) {
...
template <typename T, SizeType Dimension, class Allocator>
inline void Grid<T, Dimension, Allocator>::assign(const IndexD &size,
                                                  const T &default_value) {
  _data.assign(size.prod(), default_value);
  DopeVector<T, Dimension>::reset(_data.data(), static_cast<SizeType>(0), size);
}
...
template <typename T, SizeType Dimension, class Allocator>
inline void Grid<T, Dimension, Allocator>::assign(const SizeType size,
                                                  const T &default_value) {
  IndexD newSize = IndexD::Constant(size);
...
template <typename T, SizeType Dimension, class Allocator>
inline void Grid<T, Dimension, Allocator>::conservativeResize(
    const IndexD &size, const T &default_value) {
  ...
  DopeVector<T, Dimension> oldWindow =
      DopeVector<T, Dimension>::window(start, minSize);
...
template <typename T, SizeType Dimension, class Allocator>
inline void Grid<T, Dimension, Allocator>::conservativeResize(
    const IndexD &size, const IndexD &order, const T &default_value) {
  IndexD original_offset;
  original_offset[Dimension - 1] = 1;
  for (SizeType d = Dimension - 1; d > static_cast<SizeType>(0); --d)
    original_offset[d - 1] = size[d] * original_offset[d];
...
template <typename T, SizeType Dimension, class Allocator>
inline void Grid<T, Dimension, Allocator>::conservativeResize(
    const SizeType size, const T &default_value) {
  IndexD newSize = IndexD::Constant(size);
...
template <typename T, SizeType Dimension, class Allocator>
inline void Grid<T, Dimension, Allocator>::conservativeResize(
    const SizeType size, const IndexD &order, const T &default_value) {
  IndexD newSize = IndexD::Constant(size);
...
template <typename T, SizeType Dimension, class Allocator>
inline Grid<T, Dimension, Allocator> &Grid<T, Dimension, Allocator>::operator=(
    const Grid &o) {
  if (&o != this) {
    _data = o._data;
    DopeVector<T, Dimension>::reset(_data.data(), static_cast<SizeType>(0),
...
#ifdef DOPE_USE_RTTI
template <typename T, SizeType Dimension, class Allocator>
inline void Grid<T, Dimension, Allocator>::import(
    const DopeVector<T, Dimension> &o) {
  if (&o == this) return;

  try {
    const Grid<T, Dimension, Allocator> &oo =
        dynamic_cast<const Grid<T, Dimension, Allocator> &>(o);
    IndexD size = DopeVector<T, Dimension>::allSizes();
    for (SizeType d = static_cast<SizeType>(0); d < Dimension; ++d)
...
#endif  // INCLUDE_DOPE_VECTOR_INTERNAL_INLINES_GRID_HPP_
