...
#ifndef INCLUDE_DOPE_VECTOR_INTERNAL_INLINES_INDEX_HPP_
#define INCLUDE_DOPE_VECTOR_INTERNAL_INLINES_INDEX_HPP_
...
inline Index<Dimension>::Index(const SizeType size0, Sizes &&...sizes)
    : std::array<SizeType, Dimension>(
          {{size0, std::forward<SizeType>(sizes)...}}) {}

template <SizeType Dimension>
inline Index<Dimension>::Index(const std::initializer_list<SizeType> &il) {
  std::copy(il.begin(), il.begin() + std::min(il.size(), Dimension),
            std::array<SizeType, Dimension>::begin());
  std::fill(
      std::array<SizeType, Dimension>::begin() + std::min(il.size(), Dimension),
      std::array<SizeType, Dimension>::end(),
      std::array<SizeType, Dimension>::operator[](
          std::min(Dimension, std::min(il.size(), Dimension)) - 1));
}

template <SizeType Dimension>
template <class E>
inline Index<Dimension>::Index(
    const internal::StaticArrayExpression<E, SizeType, Dimension> &e) {
...
inline Index<Dimension>::Index(const Eigen::MatrixBase<Derived> &e) {
  static_assert(
      (Eigen::MatrixBase<Derived>::RowsAtCompileTime == Dimension &&
       Eigen::MatrixBase<Derived>::ColsAtCompileTime == 1) ||
          (Eigen::MatrixBase<Derived>::RowsAtCompileTime == 1 &&
           Eigen::MatrixBase<Derived>::ColsAtCompileTime == Dimension),
...
template <SizeType Dimension>
inline Index<Dimension> &Index<Dimension>::operator=(
    const std::initializer_list<SizeType> &il) {
  std::copy(il.begin(), il.begin() + std::min(il.size(), Dimension),
            std::array<SizeType, Dimension>::begin());
  std::fill(
      std::array<SizeType, Dimension>::begin() + std::min(il.size(), Dimension),
...
template <SizeType Dimension>
inline constexpr Index<Dimension> Index<Dimension>::Constant(
    const SizeType value) {
  return Index({{value}});
}

template <SizeType Dimension>
inline SizeType to_position(const Index<Dimension> &index,
                            const Index<Dimension> &range) {
...
template <SizeType Dimension>
inline SizeType to_positionFromOffset(const Index<Dimension> &index,
                                      const Index<Dimension> &offset) {
...
template <SizeType Dimension>
inline Index<Dimension> to_index(const SizeType position,
                                 const Index<Dimension> &range) {
...
template <SizeType Dimension>
inline Index<Dimension> to_indexFromOffset(const SizeType position,
                                           const Index<Dimension> &offset) {
  SizeType linear_position = position;
  Index<Dimension> order;
  for (SizeType i = static_cast<SizeType>(0); i < Dimension; ++i) order[i] = i;
  std::sort(order.begin(), order.end(),
            [&offset](const SizeType &l, const SizeType &r) -> bool {
...
#endif  // INCLUDE_DOPE_VECTOR_INTERNAL_INLINES_INDEX_HPP_
