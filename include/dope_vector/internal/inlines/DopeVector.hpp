...
#ifndef INCLUDE_DOPE_VECTOR_INTERNAL_INLINES_DOPEVECTOR_HPP_
#define INCLUDE_DOPE_VECTOR_INTERNAL_INLINES_DOPEVECTOR_HPP_
...
template <typename T, SizeType Dimension>
inline DopeVector<T, Dimension>::DopeVector(T *array,
                                            const SizeType accumulatedOffset,
                                            const IndexD &size)
    : _array(array),
...
template <typename T, SizeType Dimension>
inline void DopeVector<T, Dimension>::reset(T *array,
                                            const SizeType accumulatedOffset,
                                            const IndexD &size) {
  _array = array;
...
template <typename T, SizeType Dimension>
inline void DopeVector<T, Dimension>::import(
    const DopeVector<T, Dimension> &o) {
  if (&o == this) return;
...
template <typename T, SizeType Dimension>
inline void DopeVector<T, Dimension>::safeImport(
    const DopeVector<T, Dimension> &o) {
  if (&o == this) return;
  if (allSizes() != o.allSizes())
    throw std::out_of_range("Matrixes do not have same size.");
  std::unique_ptr<T[]> tempPtr(new T[o.size()]);
  DopeVector<T, Dimension> tmpDopeVector(
      tempPtr.get(), static_cast<SizeType>(0), o.allSizes());
...
template <typename T, SizeType Dimension>
inline void DopeVector<T, Dimension>::at(
    const SizeType i, DopeVector<T, Dimension - 1> &s) const {
  if (i >= _size[0]) {
...
template <typename T, SizeType Dimension>
inline DopeVector<T, Dimension - 1> DopeVector<T, Dimension>::at(
...
template <typename T, SizeType Dimension>
inline DopeVector<T, Dimension - 1> DopeVector<T, Dimension>::operator[](
    const SizeType i) const {
  DopeVector<T, Dimension - 1> s;
  at(i, s);
  return s;
...
template <typename T, SizeType Dimension>
inline typename DopeVector<T, Dimension>::iterator
DopeVector<T, Dimension>::begin() {
  return iterator(*this, static_cast<SizeType>(0), true);
}

template <typename T, SizeType Dimension>
inline typename DopeVector<T, Dimension>::iterator
DopeVector<T, Dimension>::end() {
  return iterator(*this, static_cast<SizeType>(0), false);
}

template <typename T, SizeType Dimension>
inline typename DopeVector<T, Dimension>::const_iterator
DopeVector<T, Dimension>::begin() const {
  return cbegin();
}

template <typename T, SizeType Dimension>
inline typename DopeVector<T, Dimension>::const_iterator
DopeVector<T, Dimension>::end() const {
  return cend();
}

template <typename T, SizeType Dimension>
inline typename DopeVector<T, Dimension>::const_iterator
DopeVector<T, Dimension>::cbegin() const {
  return const_iterator(*this, static_cast<SizeType>(0), true);
}

template <typename T, SizeType Dimension>
inline typename DopeVector<T, Dimension>::const_iterator
DopeVector<T, Dimension>::cend() const {
  return const_iterator(*this, static_cast<SizeType>(0), false);
}

template <typename T, SizeType Dimension>
inline typename DopeVector<T, Dimension>::iterator
DopeVector<T, Dimension>::to_iterator(const SizeType i) {
  IndexD orig = dope::to_index(iterator(*this, i, true).to_original(),
                               accumulatedOffset(IndexD::Zero()), _offset);
  return iterator(*this, i, true);
}

template <typename T, SizeType Dimension>
inline typename DopeVector<T, Dimension>::iterator
DopeVector<T, Dimension>::to_iterator_from_original(const SizeType i) {
  return iterator(*this, dope::to_index(i, _accumulatedOffset, _offset), true);
}

template <typename T, SizeType Dimension>
inline typename DopeVector<T, Dimension>::iterator
DopeVector<T, Dimension>::to_iterator(const IndexD &i) {
  return iterator(*this, i, true);
}

template <typename T, SizeType Dimension>
inline typename DopeVector<T, Dimension>::const_iterator
DopeVector<T, Dimension>::to_const_iterator(const SizeType i) const {
  return const_iterator(*this, i, true);
}

template <typename T, SizeType Dimension>
inline typename DopeVector<T, Dimension>::const_iterator
DopeVector<T, Dimension>::to_const_iterator_from_original(
    const SizeType i) const {
  return const_iterator(*this, dope::to_index(i, _accumulatedOffset, _offset),
                        true);
}

template <typename T, SizeType Dimension>
inline typename DopeVector<T, Dimension>::const_iterator
DopeVector<T, Dimension>::to_const_iterator(const IndexD &i) const {
  return const_iterator(*this, i, true);
}
...
template <typename T, SizeType Dimension>
inline void DopeVector<T, Dimension>::slice(
    const SizeType d, const SizeType i, DopeVector<T, Dimension - 1> &s) const {
  if (d >= Dimension) {
    std::stringstream stream;
  ...
  s.reset(_array + _offset[d] * i, accumulatedOffset(i, d), new_size,
          new_offset);
}

template <typename T, SizeType Dimension>
inline DopeVector<T, Dimension - 1> DopeVector<T, Dimension>::slice(
    const SizeType d, const SizeType i) const {
  DopeVector<T, Dimension - 1> s;
  slice(d, i, s);
  return s;
}
...
template <typename T, SizeType Dimension>
inline void DopeVector<T, Dimension>::permute(const IndexD &order,
                                              DopeVector &p) const {
...
template <typename T, SizeType Dimension>
inline DopeVector<T, Dimension> DopeVector<T, Dimension>::permute(
    const IndexD &order) const {
...
template <typename T, SizeType Dimension>
inline void DopeVector<T, Dimension>::window(
    const IndexD &start, const IndexD &size,
    DopeVector<T, Dimension> &w) const {
  for (SizeType d = static_cast<SizeType>(0); d < Dimension; ++d) {
    if (start[d] >= _size[d]) {
      std::stringstream stream;
    ...
    if (start[d] + size[d] > _size[d]) {
      std::stringstream stream;
      stream << "Window size " << size[d] << " is out of range [" << 0 << ", "
             << _size[d] - start[d] << ']';
      throw std::out_of_range(stream.str());
    }
  }
  ...
  w.reset(_array + (newAccumulatedOffset - _accumulatedOffset),
          newAccumulatedOffset, size, _offset);
}

template <typename T, SizeType Dimension>
inline DopeVector<T, Dimension> DopeVector<T, Dimension>::window(
    const IndexD &start, const IndexD &size) const {
...
template <typename T, SizeType Dimension>
inline void DopeVector<T, Dimension>::allSizes(
    typename DopeVector<T, Dimension>::IndexD &s) const {
  s = _size;
}

template <typename T, SizeType Dimension>
inline const typename DopeVector<T, Dimension>::IndexD &
DopeVector<T, Dimension>::allSizes() const {
  return _size;
}
...
template <typename T, SizeType Dimension>
inline void DopeVector<T, Dimension>::allOffsets(
    typename DopeVector<T, Dimension>::IndexD &o) const {
  o = _offset;
}

template <typename T, SizeType Dimension>
inline const typename DopeVector<T, Dimension>::IndexD &
DopeVector<T, Dimension>::allOffsets() const {
  return _offset;
}

template <typename T, SizeType Dimension>
inline SizeType DopeVector<T, Dimension>::accumulatedOffset(
    const SizeType i, const SizeType d) const {
  if (d >= Dimension) {
    std::stringstream stream;
    stream << "Index " << d << " is out of range [0, " << Dimension - 1 << ']';
    throw std::out_of_range(stream.str());
...
template <typename T, SizeType Dimension>
inline bool DopeVector<T, Dimension>::operator==(
    const DopeVector<T, Dimension> &r) const {
...
template <typename T>
inline DopeVector<T, 1>::DopeVector(T *array, const SizeType accumulatedOffset,
                                    const SizeType size)
    : _array(array),
      _accumulatedOffset(array ? accumulatedOffset : static_cast<SizeType>(0)),
...
template <typename T>
inline DopeVector<T, 1>::DopeVector(T *array, const SizeType accumulatedOffset,
                                    const SizeType size, const SizeType offset)
    : _array(array),
      _accumulatedOffset(array ? accumulatedOffset : static_cast<SizeType>(0)),
      _size(array ? Index1::Constant(size) : Index1::Zero()),
...
template <typename T>
inline DopeVector<T, 1>::DopeVector(T *array, const SizeType accumulatedOffset,
                                    const Index1 &size, const Index1 &offset)
    : _array(array),
      _accumulatedOffset(array ? accumulatedOffset : static_cast<SizeType>(0)),
      _size(array ? size : static_cast<const Index1 &>(Index1::Zero())),
...
template <typename T>
inline void DopeVector<T, 1>::reset(T *array, const SizeType accumulatedOffset,
                                    const SizeType size) {
  _array = array;
  _accumulatedOffset = _array ? accumulatedOffset : static_cast<SizeType>(0);
...
inline void DopeVector<T, 1>::safeImport(const DopeVector<T, 1> &o) {
  ...
  if (sizeAt(0) != o.sizeAt(0))
    throw std::out_of_range("Matrixes do not have same size.");
  std::unique_ptr<T[]> tempPtr(new T[o.size()]);
  DopeVector<T, 1> tmpDopeVector(tempPtr.get(), static_cast<SizeType>(0),
                                 o.allSizes());
  tmpDopeVector.import(o);
  import(tmpDopeVector);
}
...
template <typename T>
inline typename DopeVector<T, 1>::const_iterator DopeVector<T, 1>::begin()
    const {
  return cbegin();
}
...
template <typename T>
inline typename DopeVector<T, 1>::const_iterator DopeVector<T, 1>::cbegin()
    const {
  return const_iterator(*this, static_cast<SizeType>(0), true);
}

template <typename T>
inline typename DopeVector<T, 1>::const_iterator DopeVector<T, 1>::cend()
    const {
  return const_iterator(*this, static_cast<SizeType>(0), false);
}

template <typename T>
inline typename DopeVector<T, 1>::iterator DopeVector<T, 1>::to_iterator(
    const SizeType i) {
  return iterator(*this, i, true);
}
...
template <typename T>
inline typename DopeVector<T, 1>::const_iterator
DopeVector<T, 1>::to_const_iterator(const SizeType i) const {
  return const_iterator(*this, i, true);
}

template <typename T>
inline typename DopeVector<T, 1>::const_iterator
DopeVector<T, 1>::to_const_iterator(const Index1 &i) const {
  return const_iterator(*this, i, true);
}
...
template <typename T>
inline void DopeVector<T, 1>::permute(const Index1 &order,
                                      DopeVector<T, 1> &p) const {
  if (order[0] >= 1) {
    std::stringstream stream;
...
template <typename T>
inline void DopeVector<T, 1>::window(const Index1 &start, const Index1 &size,
                                     DopeVector<T, 1> &w) const {
  if (start[0] >= _size[0]) {
    std::stringstream stream;
  ...
  if (start[0] + size[0] > _size[0]) {
    std::stringstream stream;
    stream << "Window size " << size[0] << " is out of range [" << 0 << ", "
           << _size[0] - start[0] << ']';
    throw std::out_of_range(stream.str());
  }
  SizeType accumulatedOffset = _accumulatedOffset + _offset[0] * start[0];
  w.reset(_array + (accumulatedOffset - _accumulatedOffset), accumulatedOffset,
          size, _offset);
}

template <typename T>
inline DopeVector<T, 1> DopeVector<T, 1>::window(const Index1 &start,
                                                 const Index1 &size) const {
  DopeVector<T, 1> w;
  window(start, size, w);
  return w;
}
...
template <typename T>
inline void DopeVector<T, 1>::window(const SizeType start, const SizeType size,
                                     DopeVector<T, 1> &w) const {
  window(Index1::Constant(start), Index1::Constant(size), w);
}

template <typename T>
inline DopeVector<T, 1> DopeVector<T, 1>::window(const SizeType start,
...
inline SizeType DopeVector<T, 1>::sizeAt(const SizeType d) const {
  if (d >= static_cast<SizeType>(1)) {
    std::stringstream stream;
    stream << "Index " << d << " is out of range [0, "
...
#endif  // INCLUDE_DOPE_VECTOR_INTERNAL_INLINES_DOPEVECTOR_HPP_
