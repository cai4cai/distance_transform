...
#ifndef INCLUDE_DOPE_VECTOR_INTERNAL_INLINES_ITERATOR_HPP_
#define INCLUDE_DOPE_VECTOR_INTERNAL_INLINES_ITERATOR_HPP_
...
template <typename T, SizeType Dimension, bool Const>
inline Iterator<T, Dimension, Const>::Iterator(DopeVectorType &dope_vector,
                                               const SizeType i,
                                               const bool valid)
    : _data(dope_vector),
      _currentIndex(valid ? dope::to_index<Dimension>(i, dope_vector.allSizes())
                          : IndexD::Zero()),
      _valid(valid) {
...
inline SizeType Iterator<T, Dimension, Const>::to_original() const {
  if (!_valid) throw std::range_error("Iterator not valid.");
...
template <typename T, SizeType Dimension, bool Const>
inline const typename Iterator<T, Dimension, Const>::IndexD &
Iterator<T, Dimension, Const>::to_index() const {
  if (!_valid) throw std::range_error("Iterator not valid.");
...
template <typename T, SizeType Dimension, bool Const>
inline typename Iterator<T, Dimension, Const>::reference
Iterator<T, Dimension, Const>::operator*() const {
  if (!_valid) throw std::range_error("Iterator not valid.");
  return const_cast<reference>(_data.get().at(_currentIndex));
}

template <typename T, SizeType Dimension, bool Const>
inline typename Iterator<T, Dimension, Const>::pointer
Iterator<T, Dimension, Const>::operator->() const {
  if (!_valid) throw std::range_error("Iterator not valid.");
...
template <typename T, SizeType Dimension, bool Const>
inline typename Iterator<T, Dimension, Const>::reference
Iterator<T, Dimension, Const>::operator[](const SizeType n) const {
  return *(*this + n);
}

template <typename T, SizeType Dimension, bool Const>
inline typename Iterator<T, Dimension, Const>::reference
Iterator<T, Dimension, Const>::operator[](const IndexD &n) const {
  return *(*this + n);
}
...
template <typename T, SizeType Dimension, bool Const>
inline typename Iterator<T, Dimension, Const>::self_type &
Iterator<T, Dimension, Const>::operator++() {
  return *this += static_cast<SizeType>(1);
}

template <typename T, SizeType Dimension, bool Const>
inline typename Iterator<T, Dimension, Const>::self_type
Iterator<T, Dimension, Const>::operator++(int) {
  self_type copy(*this);
  ++*this;
  return copy;
}
...
template <typename T, SizeType Dimension, bool Const>
inline typename Iterator<T, Dimension, Const>::self_type &
Iterator<T, Dimension, Const>::operator+=(const SizeType n) {
  if (!_valid) return *this;
  Index<Dimension> size = _data.get().allSizes();
  SizeType tmp_val, carry = n;
  for (SizeType D = Dimension; D > 0 && carry != static_cast<SizeType>(0);
       --D) {
...
template <typename T, SizeType Dimension, bool Const>
inline typename Iterator<T, Dimension, Const>::self_type &
Iterator<T, Dimension, Const>::operator+=(const Index<Dimension> &n) {
  if (!_valid) return *this;
  Index<Dimension> size = _data.get().allSizes();
...
template <typename T, SizeType Dimension, bool Const>
inline typename Iterator<T, Dimension, Const>::self_type
Iterator<T, Dimension, Const>::operator+(const Index<Dimension> &n) const {
  self_type copy(*this);
  copy += n;
  return copy;
}
...
Iterator<T, Dimension, Const>::operator-=(const SizeType n) {
  ...
  for (SizeType D = Dimension;
       D > static_cast<SizeType>(0) && carry != static_cast<difference_type>(0);
       --D) {
    SizeType d = D - static_cast<SizeType>(1);
    ...
    if (tmp_val < static_cast<difference_type>(0))
      loan = std::abs(tmp_val) / static_cast<difference_type>(size[d]) +
             (std::abs(tmp_val) % static_cast<difference_type>(size[d]) !=
...
Iterator<T, Dimension, Const>::operator-=(const Index<Dimension> &n) {
  ...
  for (SizeType D = Dimension; D > static_cast<SizeType>(0); --D) {
    SizeType d = D - static_cast<SizeType>(1);
    tmp_val = static_cast<difference_type>(_currentIndex[d]) + carry -
              static_cast<difference_type>(n[d]);
    loan = static_cast<difference_type>(0);
    if (tmp_val < static_cast<difference_type>(0))
      loan = std::abs(tmp_val) / static_cast<difference_type>(size[d]) +
             (std::abs(tmp_val) % static_cast<difference_type>(size[d]) !=
...
template <typename T, SizeType Dimension, bool Const>
inline typename Iterator<T, Dimension, Const>::difference_type
Iterator<T, Dimension, Const>::operator-(const self_type &o) const {
  if (!_valid || !o._valid) throw std::range_error("Iterator not valid.");
  if (&_data.get() == &o._data.get())
    return static_cast<difference_type>(o.to_position()) -
           static_cast<difference_type>(to_position());
...
template <typename T, SizeType Dimension, bool Const>
inline bool Iterator<T, Dimension, Const>::operator==(
    const self_type &o) const {
  return &_data.get() == &o._data.get() &&
         ((!_valid && !o._valid) ||
          (_valid && o._valid && _currentIndex.isApprox(o._currentIndex)));
}
...
inline bool Iterator<T, Dimension, Const>::operator<(const self_type &o) const {
  ...
  return (_valid && !o._valid) ||
         (_valid && o._valid && to_position() < o.to_position());
}
...
#endif  // INCLUDE_DOPE_VECTOR_INTERNAL_INLINES_ITERATOR_HPP_
