...
#ifndef INCLUDE_DISTANCE_TRANSFORM_DISTANCE_TRANSFORM_H_
...
/// The DistanceTransform class provides static method to compute a distance
/// field over any multi-dimensional regularly sampled function. The dimension
/// is fixed at compile time. It is also possible to compute the index of the
/// nearest minimum of each sample.
class DistanceTransform {
 public:
  /**
   *    @brief Compute the L2-norm distance field D of a DIM-dimensional sampled
   * function f. D gets the distance from the local minima of f.
  ...
  /**
   *    @brief Compute the L2-norm distance field D of a 1-dimensional sampled
   * function f. D gets the distance from the local minima of f.
  ...
  /**
   *    @brief Compute the L2-norm distance field D of a DIM-dimensional sampled
   * function f. D gets the distance from the local minima of f. Compute also
   * the (index of the) nearest local minimum of each sample.
   *    @param f              A DIM-dimensional, regularly sampled function.
   *    @param D              The resulting distance field of f.
   *    @param I              Resulting array containing the (index of the)
   ...
   *    @param I              Resulting array containing the initial index for
   * each sample.
   */
  template <dope::SizeType DIM>
  inline static void initializeIndices(
 ...
 private:
   ...
   *    @param f             A DIM-dimensional, regularly sampled function (a
   ...
   * of this method, in lower dimenional sub-functions.
   *    @param f              A DIM-dimensional, regularly sampled function.
   *    @param D              The resulting distance field of f.
   */
  template <typename Scalar, dope::SizeType DIM>
   ...
   * Transforms of Sampled Functions" paper, performed in a mono-dimensional
   * function.
   *    @param f              A 1-dimensional, regularly sampled function.
   *    @param D              The resulting distance field of f.
   */
   ...
   *    @param D             The resulting distance field of f (a window, in
   * multi-threading).
   *    @param Ipre          Array containing the initial inedx of local minimum
   * for each sample.
   *    @param Ipost         Array containing the resulting index of local
   ...
   *    @param f              A DIM-dimensional, regularly sampled function.
   *    @param D              The resulting distance field of f.
   *    @param I              Resulting array containing the index of the local
   * minimum for each sample.
   */
  template <typename Scalar, dope::SizeType DIM>
 ...
 public:
  /**
   *    @brief Compute the square root of each individual element of a
   * DIM-dimensional array.
   *    @param m              A DIM-dimensioanl array whose element have to be
   * square rooted.
   */
  template <typename Scalar, dope::SizeType DIM>
   ...
   *    @brief Compute the square root of each individual element of a
   * 1-dimensional array.
   *    @param m              A 1-dimensioanl array whose element have to be
   * square rooted.
   */
  template <typename Scalar>
